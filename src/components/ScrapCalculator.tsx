...
export default function ScrapCalculator({ 
  ...
  return (
    <>
      ...
      <div className="bg-brand-50 p-5 rounded-2xl border border-brand-200 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <input
            type="text"
            placeholder="Employee ID / Staff Name"
            className="border p-3 rounded-xl text-sm shadow-sm font-medium"
            value={employeeId}
            onChange={(e) => setEmployeeId(e.target.value)}
          />
          <div className="flex items-center border p-2 rounded-xl bg-white shadow-sm md:col-span-2">
    ...
    {/* Print-only container: Guaranteed 1-Page Layout */}
    <div id="scrap-receipt-box" className="hidden print:block bg-white text-brand-900 text-left font-sans w-full max-w-2xl mx-auto p-2 print:p-0 print:max-h-[1020px] print:overflow-hidden break-inside-avoid">
      <div className="border border-brand-200 rounded-xl p-4 print:p-3.5 bg-white shadow-none space-y-3">
        ...
        <div className="flex justify-between items-start border-b border-brand-900 pb-2.5">
          ...
          <div className="text-right">
            ...
            <p className="text-[7.5px] font-mono text-brand-400">
              Ref ID: #{editingTransaction?.id || 'NEW'}
        ...
        <div className="bg-brand-50/80 px-2.5 py-1 rounded-lg border border-brand-200/60 flex items-center justify-between text-[8px] text-brand-600 font-mono">
          ...
          <span>Gold: <strong className="text-brand-900">${spotPrices.gold.toFixed(2)}</strong>/oz CAD</span>
          <span>Silver: <strong className="text-brand-900">${spotPrices.silver.toFixed(2)}</strong>/oz CAD</span>
          <span>Platinum: <strong className="text-brand-900">${spotPrices.platinum.toFixed(2)}</strong>/oz CAD</span>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2.5 text-[9.5px]">
          <div className="bg-brand-50/60 p-2.5 rounded-lg border border-brand-200">
            ...
            <p className="font-black text-brand-900 text-[10px] leading-tight">{customerName || 'Walk-in Client'}</p>
            {phoneNumber && (
              <p className="text-brand-700 mt-0.5 font-medium leading-tight">Phone: {phoneNumber}</p>
            )}
            {address && (
              <p className="text-brand-500 mt-0.5 leading-tight">{address}</p>
            )}
            {driversLicense && (
          ...
          <div className="bg-emerald-50/70 p-2.5 rounded-lg border border-emerald-200 text-right flex flex-col justify-between">
            <div>
              ...
              {isOfferOverridden && (
                <p className="text-[7.5px] font-mono text-amber-700 font-bold mt-0.5">
                  (Agreed Offer Override)
                </p>
              )}
            </div>
            <div className="text-[8px] text-emerald-800/80 font-mono mt-1 pt-1 border-t border-emerald-200/60 flex justify-between">
              <span>Total Weight: <strong>{items.reduce((acc, it) => acc + (Number(it.weight) || 0), 0).toFixed(2)}g</strong></span>
        ...
        <div className="space-y-1">
          <div className="flex justify-between items-center border-b border-brand-200 pb-1">
            ...
            <span className="text-[7.5px] font-mono text-brand-500">
              {items.filter(i => Number(i.weight) > 0).length} lot(s) evaluated
            </span>
          </div>
              ...
              {Number(stoneRemovalQty) > 0 && (
                <tr className="bg-red-50/40 text-red-700 font-medium">
                  ...
                  <td colSpan={4} className="py-1 px-1.5">
                    Stone extraction & bench refining fee ({stoneRemovalQty} stone{Number(stoneRemovalQty) > 1 ? 's' : ''} @ $5.00/ea)
                  </td>
                  <td className="py-1 px-1.5 font-mono font-bold text-right text-red-700">
                    -${(Number(stoneRemovalQty) * 5).toFixed(2)}
                  </td>
                </tr>
              )}
            </tbody>
          ...
          <div className="grid grid-cols-2 gap-4 items-end pt-1">
            <div className="text-center flex flex-col items-center">
              ...
              <p className="text-[7px] text-brand-400">
                {customerName || 'Authorized Seller'}
              </p>
            </div>
